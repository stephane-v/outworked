...
import { getSoundsEnabled, setSoundsEnabled, getDesktopNotificationsEnabled, setDesktopNotificationsEnabled } from '../lib/sounds';
...
export default function NotificationCenter({ notifications, onDismiss, onDismissAll, onApprovalResponse, onNavigateToAgent }: NotificationCenterProps) {
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setOpen(false)}>
          <div className="bg-slate-800 border border-slate-600 rounded-lg w-[420px] max-h-[70vh] shadow-xl flex flex-col" onClick={e => e.stopPropagation()}>
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700">
              <h3 className="text-sm font-pixel text-white">🔔 Notifications</h3>
              <div className="flex items-center gap-2">
                {notifications.length > 0 && (
                  <button onClick={onDismissAll} className="text-[9px] font-pixel text-slate-500 hover:text-slate-300">
                    Clear all
                  </button>
                )}
            ...
            <div className="flex items-center gap-3 px-4 py-2 border-b border-slate-700/50">
              <button
                onClick={() => { const v = !soundsOn; setSoundsOn(v); setSoundsEnabled(v); }}
              ...
              <button
                onClick={() => { const v = !desktopOn; setDesktopOn(v); setDesktopNotificationsEnabled(v); }}
            ...
            <div className="flex-1 overflow-y-auto">
              {notifications.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-[11px] font-pixel text-slate-500 mt-2">No notifications yet</p>
                </div>
              )}
              {notifications.map(n => (
                <div
                  ...
                  <div className="flex items-start gap-2">
                    <span className="text-sm mt-0.5 shrink-0">
                      {n.type === 'approval' ? '🔒' :
                       ...
                       n.type === 'agent-stuck' ? '⚠️' :
                       n.type === 'orchestration-done' ? '🏁' : 'ℹ️'}
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-[11px] font-pixel text-white">{n.title}</p>
                        {n.agentName && (
                          <button
                      ...
                      <p className="text-[9px] text-slate-600 mt-1">{formatTimestamp(n.timestamp)}</p>

                      {/* Approval actions */}
                      {n.type === 'approval' && !n.read && n.permissionReqId !== undefined && (
                        <div className="flex gap-2 mt-2">
                          <button
                            onClick={() => onApprovalResponse(n.id, n.permissionReqId!, true)}
                            className="btn-pixel text-[10px] bg-emerald-700 hover:bg-emerald-600 text-white px-3 py-0.5"
                          >
                            ✓ Allow
                          </button>
                          <button
                            onClick={() => onApprovalResponse(n.id, n.permissionReqId!, false)}
...
export function NotificationToast({ notification, onDismiss }: { notification: AppNotification | null; onDismiss: () => void }) {
  useEffect(() => {
  ...
  const borderColor = notification.type === 'approval' ? 'border-amber-500/50' :
    notification.type === 'task-complete' || notification.type === 'orchestration-done' ? 'border-emerald-500/50' :
    notification.type === 'agent-stuck' ? 'border-red-500/50' : 'border-slate-500/50';

  const bgColor = notification.type === 'approval' ? 'bg-amber-950/90' :
    notification.type === 'task-complete' || notification.type === 'orchestration-done' ? 'bg-emerald-950/90' :
    notification.type === 'agent-stuck' ? 'bg-red-950/90' : 'bg-slate-950/90';

  return (
    <div className={`absolute top-14 right-4 z-30 rounded-lg border shadow-xl px-3 py-2 max-w-xs backdrop-blur-sm animate-slide-up ${borderColor} ${bgColor}`}>
      <div className="flex items-start gap-2">
        <span className="text-sm">
          {notification.type === 'approval' ? '🔒' :
           notification.type === 'task-complete' ? '✅' :
           notification.type === 'agent-stuck' ? '⚠️' :
           notification.type === 'orchestration-done' ? '🏁' : 'ℹ️'}
        </span>
        <div className="flex-1 min-w-0">
          <p className="text-[11px] font-pixel text-white">{notification.title}</p>
          <p className="text-[10px] text-slate-400 mt-0.5 truncate">{notification.body}</p>
        </div>
